...
class Game
{
	...
	public $source = null;
	public $game_type = 'html5';
	public $editor_type = 'default';
	public $thumb_1 = null;
	public $thumb_2 = null;
	public $thumb_small = '';
	public $url = null;
	...
	public function __construct($data = array())
	{
		...
		if (isset($data['title'])) {
			if(get_setting_value('disable_iconv')){
				...
				//
				// Temporary disabled since v1.9.2, causing encoding bug for unicode language characters
				// This iconv() issue can be fixed by set DB collation to utf8_general_ci
		...
		if (isset($data['description'])) {
			if (get_setting_value('disable_iconv')) {
				...
				// Check if mbstring functions exist
				if (function_exists('mb_detect_encoding')) {
					// Original code with mbstring functions
					...
					// Just use iconv directly for basic sanitization
					$this->description = iconv("utf-8", "utf-8//ignore", $data['description']);
				} else {
					// No encoding functions available
					$this->description = $data['description'];
				}
			}
		}
		...
		if (isset($data['is_premium'])) {
            $this->is_premium = filter_var($data['is_premium'], FILTER_VALIDATE_BOOLEAN) ? true : false;
        }
	}
	...
	public static function getListByType($game_type, int $amount = 20, int $page = 0){
		$additional_condition = '';
		if(defined('IS_VISITOR_PAGE')){
			if(get_setting_value('hide_pc_on_mobile') && is_mobile_device()){
				$additional_condition = 'AND is_mobile = 1';
		...
		$conn = open_connection();
		$sql = "SELECT * FROM games WHERE game_type = :game_type
			AND published = 1 $additional_condition ORDER BY id DESC LIMIT $amount OFFSET $page";

		$st = $conn->prepare($sql);
		$st->bindValue(":game_type", $game_type, PDO::PARAM_STR);
		$st->execute();
		$row = $st->fetchAll();
		$list = array();
		foreach ($row as $item) {
		...
		$st = $conn->prepare($sql);
		$st->bindValue(":source", $game_type, PDO::PARAM_STR);
		$st->execute();
	...
	public static function searchGameMultilingual($keyword, int $amount = 20, int $page = 0) {
		global $lang_code;
		$additional_condition = '';
		if(defined('IS_VISITOR_PAGE')){
			if(get_setting_value('hide_pc_on_mobile') && is_mobile_device()){
				$additional_condition = 'AND g.is_mobile = 1';
			}
		}
		...
		// Get games that match in the original title/description
		$sql = "SELECT g.* FROM games g WHERE 
				(g.title LIKE :keyword OR g.description LIKE :keyword) 
				AND g.published = 1 $additional_condition";
		
		// Add search in translations if using a non-English language
		$translation_join = '';
		$translation_condition = '';
		if ($lang_code != 'en') {
			$translation_join = "LEFT JOIN (
				SELECT content_id, translation 
				FROM translations 
				WHERE content_type = 'game' 
				AND language = :lang_code 
				AND field IN ('title', 'description')
			) t ON g.id = t.content_id";
			
			$translation_condition = "OR t.translation LIKE :keyword";
			$sql = "SELECT DISTINCT g.* FROM games g $translation_join 
					WHERE ((g.title LIKE :keyword OR g.description LIKE :keyword) $translation_condition)
					AND g.published = 1 $additional_condition";
		}
		...
		$st = $conn->prepare($sql);
		$st->bindValue(":keyword", '%' . $keyword . '%', PDO::PARAM_STR);
		$st->bindValue(":limit", $amount, PDO::PARAM_INT);
		$st->bindValue(":offset", $page, PDO::PARAM_INT);
		
		if (!empty($translation_condition)) {
			$st->bindValue(":lang_code", $lang_code, PDO::PARAM_STR);
		}
		...
		$count_sql = "SELECT COUNT(DISTINCT g.id) FROM games g $translation_join
					 WHERE ((g.title LIKE :keyword OR g.description LIKE :keyword) $translation_condition)
					 AND g.published = 1 $additional_condition";
		
		$count_st = $conn->prepare($count_sql);
		$count_st->bindValue(":keyword", '%' . $keyword . '%', PDO::PARAM_STR);
		
		if (!empty($translation_condition)) {
			$count_st->bindValue(":lang_code", $lang_code, PDO::PARAM_STR);
		}
		
		$count_st->execute();
		$totalRows = $count_st->fetchColumn();
	...
	public function isPremium()
    {
        if ($this->is_premium === null) {
            $conn = open_connection();
            $sql = "SELECT is_premium FROM games WHERE id = :id LIMIT 1";
            $st = $conn->prepare($sql);
            $st->bindValue(":id", $this->id, PDO::PARAM_INT);
            $st->execute();
            $row = $st->fetch();
            $this->is_premium = $row ? filter_var($row['is_premium'], FILTER_VALIDATE_BOOLEAN) : false;
        }
    ...
    {
        if (is_null($this->id)) {
            trigger_error("Game::setPremium(): Attempt to set premium status on a Game object that does not have its ID property set.", E_USER_ERROR);
        }

        $conn = open_connection();
        $sql = "UPDATE games SET is_premium = :is_premium WHERE id = :id";
        
        $st = $conn->prepare($sql);
        $st->bindValue(":id", $this->id, PDO::PARAM_INT);
        $st->bindValue(":is_premium", $value ? 1 : 0, PDO::PARAM_INT);
        $st->execute();
	...
	public function insert()
	{
		...
		try {
			$sql = 'INSERT INTO games ( createdDate, title, description, instructions, category, source, game_type, editor_type, thumb_1, thumb_2, thumb_small, url, width, height, slug, tags, views, upvote, downvote, data, extra_fields, is_mobile, published ) 
					  VALUES ( :createdDate, :title, :description, :instructions, :category, :source, :game_type, :editor_type, :thumb_1, :thumb_2, :thumb_small, :url, :width, :height, :slug, :tags, 0, 0, 0, :data, :extra_fields, :is_mobile, :published )';
			$st = $conn->prepare($sql);
			...
			$st->bindValue(":game_type", $this->game_type, PDO::PARAM_STR);
			$st->bindValue(":editor_type", $this->editor_type, PDO::PARAM_STR);
	...
	{
		apply_admin_filters('pre_game_update', $this);
		...
		try {
			...
			apply_admin_filters('after_game_update', $this);
			//
			if (function_exists('log_action')) {
				log_action(
					'update',
	...
	{
		apply_admin_filters('pre_game_delete', $this);
		...
		apply_admin_filters('after_game_delete', $this);
		//
		if (function_exists('log_action')) {
			log_action(
				'delete',
