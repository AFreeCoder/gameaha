...
if($base_taxonomy === 'game'){
	// Default filter for game description
	add_filter('single_game_description', function($game) {
		if($game->editor_type != 'default'){
			return $game->description;
		}
		return nl2br($game->description);
	});
	// Default filter for game instructions
	add_filter('single_game_instructions', function($game) {
		if($game->editor_type != 'default'){
			return $game->instructions;
		}
		return nl2br($game->instructions);
	});
}

if($base_taxonomy === 'post'){
	// Default filter for post content
	add_filter('single_post_content', function($post) {
		if(isset($post->editor_type) && $post->editor_type != 'default'){
			return $post->content;
		}
		return nl2br($post->content);
	});
}
...
function the_lang_input() {
	// Output the language hidden input field for search forms if multilingual is enabled
    if (PRETTY_URL && get_setting_value('lang_code_in_url')) {
        global $lang_code;
        echo '<input type="hidden" name="lang" value="' . $lang_code . '">';
    }
}
