...
if ($thumb_method == 'upload') {
	...
	if (isset($_FILES['thumb_upload_1']) && $_FILES['thumb_upload_1']['error'] == UPLOAD_ERR_OK) {
		...
		if ($thumb_upload_1_size > 1048576) {
			...
			$move_uploaded_thumbnail_files['thumb_upload_1'] = $thumb_1_target;
	...
	if (isset($_FILES['thumb_upload_2']) && $_FILES['thumb_upload_2']['error'] == UPLOAD_ERR_OK) {
		...
		if ($thumb_upload_2_size > 1048576) {
			...
			$move_uploaded_thumbnail_files['thumb_upload_2'] = $thumb_2_target;
...
if ($uploadOk == 0) {
	...
	} else if(count($move_uploaded_thumbnail_files)){
		move_uploaded_file($_FILES['thumb_upload_1']['tmp_name'], $move_uploaded_thumbnail_files['thumb_upload_1']);
		move_uploaded_file($_FILES['thumb_upload_2']['tmp_name'], $move_uploaded_thumbnail_files['thumb_upload_2']);
	}
