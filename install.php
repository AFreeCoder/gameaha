				...
				if(!file_exists('db/tables.sql')){
					...
					if(isset($_POST['db_name'])){
						function create_tables($conn){
							...
							// Execute the query using exec() to run multiple statements
							$conn->exec($query);
						}
						...
						if($db_name != $_POST['db_name'] || $db_host != $_POST['db_host'] || $db_user != $_POST['db_user'] || $db_password != $_POST['db_password']){
							...
							echo '<p><a href="install.php">Try Again</a></p>';
						} else {
							try {
								$conn = new PDO("mysql:host=".$db_host.";port=8306;dbname=".$db_name, $db_user, $db_password);
								...
								echo '<div class="alert alert-danger" role="alert">Failed. Can\'t connect to database: ' . $e->getMessage() . '</div>';
								echo '<p><a href="install.php">Try Again</a></p>';
							};
							$conn = null;
						}
						...
						if($admin_user == $admin_user_ori){
							...
							$filecontent = str_replace('db_port', $_SESSION['db_port'], $filecontent);
							...
							$conn = new PDO("mysql:host=".$_SESSION['db_host'].";port=8306;dbname=".$_SESSION['db_name'], $_SESSION['db_user'], $_SESSION['db_password']);
							$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
