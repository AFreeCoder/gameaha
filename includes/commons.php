...
function get_permalink($type, $slug = '', $arrs = [], $lang = null){
	...
	*/
	// Note : parameter $lang used for search url with lang url parameter
	...
	if(PRETTY_URL && ($slug || $type == 'search')){
	...
	if($type == 'game'){
		...
		if( PRETTY_URL ){
			return DOMAIN . $lang_id . $end_slash;
		} else {
			return DOMAIN . 'index.php?viewpage=homepage';
		}
		...
		if( PRETTY_URL ){
			return DOMAIN . str_replace('//', '/', $lang_id.$custom_type.'/' . $slug . $params . $end_slash);
		} else {
...
function html_purify($html_content) {
    ...
    $config->set('HTML.Allowed', 'p,br,b,i,em,strong,u,a[href|target],ul,ol,li,span,div,h1,h2,h3,h4,h5,h6,img[src|alt|width|height],table,tr,td,th,thead,tbody');
    $config->set('Attr.AllowedFrameTargets', array('_blank', '_self'));
    
    // Add security for target="_blank"
...
function get_admin_warning(){
	...
	if (file_exists(ABSPATH . 'error_log')) {
	    if (filesize(ABSPATH . 'error_log') > 500000) { // Greater than 500 KB
	        array_push($results, 'Large error_log file detected in root directory. Consider deleting or moving it as it may cause site performance issues.');
	    }
	}
	return $results;
}
...
function has_admin_access($page_name = null, $page_slug = null){
...
function show_forbidden_alert(){
	// Used for crew who don't have permission to specific page
	show_alert('You don\'t have permission to access this page!', 'warning');
}
